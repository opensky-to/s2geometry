namespace OpenSky.S2Geometry.Tests
{
    ...
    {
        public Random rand { get; } = new Random(123456);
        ...
        {
            Assert.IsTrue(a + error > b);
            Assert.IsTrue(a < b + error);
        }
        ...
        protected void checkCovering(IS2Region region, S2CellUnion covering, bool checkTight, S2CellId id)
        {
            ...
            if (!region.MayIntersect(new S2Cell(id)))
            {
                ...
                if (checkTight)
                {
                    Assert.IsTrue(!covering.Intersects(id));
                }
            }
            else if (!covering.Contains(id))
            {
                ...
                Assert.IsTrue(!region.Contains(new S2Cell(id)));
